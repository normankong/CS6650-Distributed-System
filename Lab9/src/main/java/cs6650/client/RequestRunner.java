...
public class RequestRunner {
  ...
  private static final String URL_PARAM = "/skiers/999999999/seasons/20/days/%DAY%/skiers/%SKIER_ID%";
  ...
  public RequestRunner(String desc, int numThreads, int numSkiers, int numRun, int numLifts, int startTime, int endTime, int day, String url) {
    this.desc = desc;
  ...
  public void execute() throws InterruptedException {
    ...
    for (int i = 0; i < numThread; i++) {
      ...
      list.add(new ThreadRunner(desc, skierId, time, numRun, numLifts, day, url, partialCountDownLatch, totalCountDownLatch));
    }
...
//        System.out.printf("%s : %d / %d (ms) = %f tps\n", desc, numThread * numRun, totalAfter - totalBefore, tps);
        ...
        System.out.printf("%s wall\t%d\n", desc, totalAfter - totalBefore);
  ...
  private static class ThreadRunner extends Thread {
    ...
    private ThreadRunner(String desc, int skierId, int time, int numRun, int numLifts, int day, String url, CountDownLatch partialCountDownLatch, CountDownLatch totalCountDownLatch) {
      this.desc = desc;
